...
namespace mathematics {
	 ...
	 *			   the range from -1 to 1, you can use this method (see
	 *also 'another' approximation: anotherApproximatePower()). This method is
	 *based on the algorithm used in the 2005 game Quake III Arena. He raised
	 *the number x to the power -0.5, i.e. found the value:
	 *\f$\frac{1}{\sqrt{x}}\f$
	 *
	 ...
	 *			   accuracy drops rapidly as the absolute value of the
	 *degree increases and remains acceptable in the range [-10, 10] (see also
	 *'old' approximation: oldApproximatePower()).
	 *
	 * @param[in]  base		 The base
	 * @param[in]  exponent	 The exponent
	 *
	 ...
	 *			   until numbers are raised to a power of 134217728,
	 *Speed increase: ~7.5 times on average, the advantage remains until the
	 *numbers are raised to the power of 134217728. Error: none, but it is worth
	 *noting that the multiplication operation is not associative for floating
	 *point numbers, i.e. 1.21 * 1.21 is not the same as 1.1 * 1.1 * 1.1 * 1.1,
	 *however, when compared with standard functions, errors do not arise, as
	 *mentioned earlier. Restrictions: the degree must be an integer not less
	 *than 0
	 *
	 * @param[in]  base		 base
	 ...
	 *			   contains checks for special input data. Without them,
	 *the code runs only 10% faster, but the error increases tenfold (especially
	 *when using negative powers).
	 *
	 * @param[in]  base		 The base
	 * @param[in]  exponent	 The exponent
	 *
	...
	/**
	 * @brief	   Getting the modulus of a number without a comparison
	 * operation
	 *
	 * @param[in]  x	 number
	 *
