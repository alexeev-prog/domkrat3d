...
namespace physics::kinematics {
     ...
     *
     * This function is based on the kinematics equation: s = u*t + 0.5 * a *
     *t^2 where:
     *	+ s - final position (m)
     *	+ u - start speed (m/s)
     *	+ a - acceleration (m/s^2)
     ...
     *
     * Equally variable motion is a motion in which, over any equal periods of
     * time, a material point changes its speed by the same amount. Acceleration
     * is calculated by the formula \f$a =
     * \frac{V - V_{0}}{t}\f$, where v0 and v are the initial and final speed of
     * the body, t is the time of movement
     *
     ...
     *
     * To calculate speed, you can use the formula V0 + a * t, where v is speed,
     * t is time.
     *
     * @param[in]  end_speed	 The end speed
     * @param[in]  acceleration	 The acceleration
